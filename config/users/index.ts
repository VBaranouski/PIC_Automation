...
function getUserFromEnvironmentVariables(env: EnvironmentName, role: UserRole): UserCredentials | undefined {
  const login = process.env[envKey(env, role, 'LOGIN')] || process.env.PICASSO_TEST_LOGIN || process.env.PICASSO_LOGIN;
  const password = process.env[envKey(env, role, 'PASSWORD')] || process.env.PICASSO_TEST_PASSWORD || process.env.PICASSO_PASSWORD;
  const name = process.env[envKey(env, role, 'NAME')] || process.env.PICASSO_TEST_USER_NAME || process.env.PICASSO_USER_NAME || role;
...
export function getUser(role: UserRole, envName?: string): UserCredentials {
  ...
  if (!user) {
    ...
    throw new Error(
      ...
      `Provide local config/users/${env}.users.ts or set PICASSO_TEST_LOGIN/PICASSO_TEST_PASSWORD, ` +
      `PICASSO_LOGIN/PICASSO_PASSWORD, ` +
