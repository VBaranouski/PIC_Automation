...
| products | Product Lifecycle | `products` | [feature-registry/products.md](feature-registry/products.md) | product-lifecycle, product-release-inactivation | `tests/products/` | `src/pages/new-product.page.ts` |
| releases | Release Workflow (7-stage) | `releases` | [feature-registry/releases.md](feature-registry/releases.md) | release-workflow, questionnaire, process-requirements, product-requirements, csrr, fcsr-decision, actions-management, data-protection, delegated-requirements-traceability | `tests/releases/` | `src/pages/release-detail.page.ts` |
...
| backoffice | Backoffice Admin | `backoffice` | [feature-registry/backoffice.md](feature-registry/backoffice.md) | roles-permissions, roles-delegation, informative-banner, sast-tools-config | — | — |
...
- `product`, `registration`, `product detail`, `status mapping`, `inactivat`, `inactive product`, `inactive release` → **products**
...
- `admin`, `backoffice`, `role`, `delegation`, `privilege`, `banner`, `sast`, `severity input`, `sca tool` → **backoffice**
- `jira`, `jama`, `azure devops`, `integration`, `webhook`, `data extraction`, `data ingestion`, `intel ds`, `informatica` → **integrations**
- `email`, `notification`, `task trigger`, `maintenance` → **other**
