...
description: Use when writing or modifying any TypeScript file (.ts, .tsx, .mts), reviewing TypeScript code quality, implementing type-safe patterns, discussing code style, or handling async patterns, imports, UI theming, or performance in TypeScript projects.
---
...
- Avoid implicit `any`; explicitly type when necessary
...
- Uses `simple-import-sort/imports` and `consistent-type-imports` (`fixStyle: 'separate-type-imports'`)
- Always use `import type { ... }` for type-only imports — NOT `import { type ... }` inline syntax
